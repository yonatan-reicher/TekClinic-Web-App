...
const PatientFormModal: React.FC<{
    opened: boolean, onClose: () => void, session: Session, refetch: (options?: RefetchOptions) => Promise<QueryObserverResult<{
        items: Patient[];
        count: number;
    }, Error>>,
    setModalOpened: Dispatch<SetStateAction<boolean>>,
    setPatientCreated: Dispatch<SetStateAction<boolean>>,
}>
    = ({ opened, onClose, session, refetch, setModalOpened, setPatientCreated }) => {
        ...
        const form = useForm({
            ...
            validate: {
                ...
                name: (value) => {
                    ...
                    const name_no_space = value.replace(/\s/g, '');
                    if (!validator.isAlpha(name_no_space)) {
                        return 'Invalid name';
                    }
                    return null;
                },
                ...
                phone_number: (value) => {
                    if (value != '' && !validator.isMobilePhone(value, 'any', { strictMode: true })) {
                        return 'Invalid phone number';
                    }
                    return null;
                },
            ...
            <Modal opened={opened} onClose={onClose} title="Patient Information Form">
                <form onSubmit={form.onSubmit((values) => { handleSubmit({ ...values, emergency_contacts: emergencyContacts }) })}>
                    ...
                    {emergencyContacts.map((contact, index) => (
                        <Group key={'group' + `${index}`} position="apart">
                            <TextInput
                                key={form.key('emergency_contacts.' + `${index}` + '.name')}
                                label="Emergency Contact Name"
                                placeholder="Alexa Smith"
                                value={contact.name}
                                // {...form.getInputProps('emergency_contacts_' + `${index}_` + '.name')}
